        ...
        if (data.code==1){
            ...
            switch (action)
            {
                ...
                if ( !empty(data.details.profile_photo_url)){
                    var image='<img src="'+ data.details.profile_photo_url +'" />';
                ...
                if ( !empty(data.details.drop_address)){
                    ...
                    $(".dropoff_contact_name_details").html( data.details.dropoff_contact_name );
                    $(".dropoff_contact_number_details").html( data.details.dropoff_contact_number );
                    $(".drop_address_details").html( data.details.drop_address );                   
                ...
                if ( !empty(data.details.tracking_link)){
                    ...
                    $(".tracking_link").attr("href",data.details.tracking_link);
                    $(".tracking_link").html(data.details.tracking_link);
                } else {
                ...
                if ( data.details.ratings>0){
                    $(".ratings_wrap").show();
                    $(".ratings_raty").attr("data-score", data.details.ratings );                   
                    initRating2();
                    if(!empty(data.details.rating_comment)){
                       $(".rating_comment").html( data.details.rating_comment );
                    }
                } else {
                    $(".ratings_wrap").hide();
                }
                ...
                $("input[name='trans_type'][value='"+data.details.trans_type+"']").prop('checked', true);
                ...
                if ( !empty(data.details.drop_address)){
                    ...
                    $("#dropoff_task_lat").val( data.details.dropoff_task_lat );
                    $("#dropoff_task_lng").val( data.details.dropoff_task_lng );

                    
                    map_dropoff.setCenter( data.details.dropoff_task_lat , data.details.dropoff_task_lng );
                    ...
                    var map_dropoff_marker = map_dropoff.addMarker({
                         lat: data.details.dropoff_task_lat ,
                         lng: data.details.dropoff_task_lng ,
                         draggable: true,                        
                    });
                    ...
                    map_dropoff_marker.addListener('dragend',function(event) {           
                         ...
                         if($('.new-task-submit').is(':visible')) {
                            convertLatLongToAddressDropOff( event.latLng.lat() , event.latLng.lng() );
                         }
                ...
                case "getContactInfo":
                  ...
                  setMarkerEdit( data.details.addresss_lat , data.details.addresss_lng ,'addresss_lat','addresss_lng' );
                ...
                case "loadContactInfo":
                   ...
                   setMarkerByAddress( $("#delivery_address").val() );
                break;
...
function tplTaskHistory(data)
{
    ...
    $.each( data , function( key, val ) {     
        ...
        html+='<div class="row">';
           ...
           if (!empty(val.reason)){
              html+='<p class="text-muted">'+jslang.reason+': '+val.reason+'</p>';
           }
           
           if (!empty(val.notes)){
                var json = jQuery.parseJSON(val.notes);
                var html1 = '';
                ...
                for (var key in json) {
                    ...
                    var value = +key + ". " + json[key]+' </br>';
                     html1 += value;
                    
                    }
                }
                html+='<p class="text-muted">'+jslang.survey+': </br>'+html1+'</p>';
           ...
           if (!empty(val.photo_name_url)){
               ...
               html+='<img class="customer-signature" src="'+val.photo_name_url+'">';
               html+='</a>';
               html+='</p>';
           }
           ...
           if (!empty(val.receive_by)){
              html+='<p class="text-muted">'+jslang.receive_by+': '+val.receive_by+'</p>';
           }
           ...
           html+='<div class="col-md-4">';
             ...
             if (!empty(val.driver_location_lat)){
                 ...
                 html+='<a href="javascript:;" class="show-location-map" data-lat="'+val.driver_location_lat+'" data-lng="'+val.driver_location_lng+'" data-taskid="'+val.task_id+'"  >'+jslang.location_on_map+'</a>';
             }
           html+='</div>';
         html+='</div> ';
         html+='</div>';
...
$(document).ready(function(){
    ...
    if ( $(".contact_address").exists() ){                              
       ...
       .bind("geocode:result", function(event, result){                         
            ...
            setMarker( $(".contact_address").val() , 'addresss_lat','addresss_lng' );
       });
    }
...
$(document).ready(function(){
    ...
    $('.new-contact').on('shown.bs.modal', function (e) {       
        ...
        if ( !empty($("#id").val())){          
           initContactMap();
           callAjax("getContactInfo", "contact_id="+$("#id").val() ,  $("#frm .orange-button") ) ;
        } else {
            initContactMap();
        }
    });
...
{       
    ...
    task_remaining_ajax = $.ajax({
     ...
     complete: function(data) {                 
        task_remaining_ajax= (function () { return; })();
        dump( 'Completed');     
     },
     success: function (data) {   
           dump(data);
           ...
           clearInterval(task_remaining_handle);                    
           task_remaining_handle = setInterval(function(){getRemainingTask()}, 20000);  
        ...
        if (status == 'OK') {
          ...
          map_contact_marker.addListener('dragend',function(event) {             
             ...
             if($('.contact-submit').is(':visible')) {
                convertLatLongToAddressGlobal( event.latLng.lat() , event.latLng.lng() , 'contact_address' );
             }
