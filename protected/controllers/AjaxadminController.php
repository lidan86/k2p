...
class AjaxadminController extends CController
{
    ...
    public function actionaddPlans()
    {
    	...
    	$params['price']=is_numeric($params['price'])?$params['price']:0;
    	$params['promo_price']=is_numeric($params['promo_price'])?$params['promo_price']:0;
    	
    	if(!isset($this->data['with_sms'])){
    		$params['with_sms']=2;
    	}    	
    	if(!isset($this->data['with_broadcast'])){
    ...
    {    	
    	if ( $res=AdminFunctions::getAdminByUsername($this->data['username'])){    		
    		...
    		} else $this->msg=t("Email address is empty");
    	} else $this->msg=t("Username doest not exist");
    	$this->jsonResponse();
    }
    ...
    {    	
    	if ( $this->data['password']==$this->data['cpassword']){
    		...
    		if ($res=AdminFunctions::getAdminByPassword($token)){
    			...
    			} else $new_password=md5($this->data['password']);
    			...
    			} else $this->msg=t("Password cannot be updated this time please try again");
    			
    		} else $this->msg=t("Token is not valid");
    	} else $this->msg=t("Password does not match with the confirm password");
    	$this->jsonResponse();
    }
    ...
    public function actionserviceslist()
    {
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$status="<div class=\"tag $stats\">".t($val['status'])."</div>";
				$status.=AdminFunctions::prettyDate($val['date_created']);
				$actions=AdminFunctions::generateActionsList('services_id',$val['services_id'],'services',
				'services-new');
    ...
    public function actionaddServices()
    {
    	...
    	if (isset($this->data['id'])){    		
    		...
    		if ( $this->db->insertData("{{services}}",$params)){
    			...
    			$this->code=1;
    			$this->msg=t("Successful");
    			$this->details=Yii::app()->createUrl('admin/services-new',
    			array( 'id'=>$last_id,'msg'=>$this->msg)
    			);
    		} else $this->msg=t("Something went wrong cannot insert records");
    	}
    ...
    {    	
    	...
    	if (isset($this->data['ids'])){
    		...
    		$this->code=1; $this->msg=t("Successfully updated");
    ...
    public function actionPromoCodeList()
    {
		...
		if ( $res=$this->db->rst($stmt)){
			...
			foreach ($res as $val) {			
				...
				$status="<div class=\"tag $stats\">".t($val['status'])."</div>";
				$status.=AdminFunctions::prettyDate($val['date_created']);
				$actions=AdminFunctions::generateActionsList('promo_code_id',$val['promo_code_id'],'promo_code',
				'promocode-new');
    ...
    public function actionAddPromoCode()
    {
    	...
    	if (isset($this->data['id'])){    		
    		...
    		} else $this->msg=t("Promo code name already exist");
    	} else {    	
    		if ( !AdminFunctions::isPromoCodeExist($this->data['promo_code_name'])){
	    		if ( $this->db->insertData("{{promo_code}}",$params)){
	    			...
	    			$this->code=1;
	    			$this->msg=t("Successful");
	    			$this->details=Yii::app()->createUrl('admin/promocode-new',
	    			array( 'id'=>$last_id,'msg'=>$this->msg)
	    			);
	    		} else $this->msg=t("Something went wrong cannot insert records");
    		} else $this->msg=t("Promo code name already exist");
