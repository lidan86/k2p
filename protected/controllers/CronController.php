...
class CronController extends CController
{
			...
			foreach ($res as $val) {
				...
				if (!empty($val['device_id'])){
					if(!empty($api_key)){
					   ...
					   if (isset($_GET['debug'])){dump($message);}
    ...
    {    	
    	file_get_contents(websiteUrl()."/cron/processpush");
    	sleep(1);
    	file_get_contents(websiteUrl()."/cron/autoassign");
    	sleep(1);
    	file_get_contents(websiteUrl()."/cron/processautoassign");
    	sleep(1);
    	file_get_contents(websiteUrl()."/cron/checkautoassign");
    	sleep(1);
    	file_get_contents(websiteUrl()."/cron/checkcustomerexpiry");
    	sleep(1);
    	file_get_contents(websiteUrl()."/cron/processbroadcast");    	
    	sleep(1);    	
    	file_get_contents(websiteUrl()."/cron/clearagenttracking");    	
    }
