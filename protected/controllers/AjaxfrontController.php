			...
			if ($res['driver_id']>0){				
				if ( $res['trans_type']=="delivery"){					
					if(!empty($res['drop_address'])){
						if ( $d1=Driver::getTaskDistance( $res['driver_location_lat'], $res['driver_location_lng'],
						    $res['dropoff_task_lat'], $res['dropoff_task_lng'] , $travel_mode )){						    	
						    	$d1_resp=explode(" ",$d1);						    	
						    	if(is_array($d1_resp) && count($d1_resp)>=1){
							    	if ( $d2=Driver::getTaskDistance( $res['dropoff_task_lat'], $res['dropoff_task_lng'],
									    $res['task_lat'], $res['task_lng'] )){								    	
									    	$d2_resp=explode(" ",$d2);								    	
									    	$distance=$d1_resp[0]+$d2_resp[0]." ".$d2_resp[1];
									}
						    	}
						}
					} else {
					...
					if(!empty($res['drop_address'])){
						if ( $d1=Driver::getTaskDistance( $res['driver_location_lat'], $res['driver_location_lng'],
						    $res['task_lat'], $res['task_lng'] , $travel_mode )){						    	
						    	$d1_resp=explode(" ",$d1);						    	
						    	if(is_array($d1_resp) && count($d1_resp)>=1){
							    	if ( $d2=Driver::getTaskDistance( $res['task_lat'], $res['task_lng'],
									    $res['dropoff_task_lat'], $res['dropoff_task_lng'] )){								    	
									    	$d2_resp=explode(" ",$d2);								    	
									    	$distance=$d1_resp[0]+$d2_resp[0]." ".$d2_resp[1];
									}
						    	}
